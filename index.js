...
async function run() {
  try {
    ...
    const dokanTransactionsCollection = client
      .db("dokantransactiondb")
      .collection("dokantransaction");
    const dailyTransactionsCollection = client
      .db("dailytransactiondb")
      .collection("dailytransaction");
    const homeproductsCollection = client
      .db("homeproducts")
    ...
    app.post("/dokantransactions", async (req, res) => {
      try {
        ...
        // Insert into MongoDB
        const result =
          await dokanTransactionsCollection.insertOne(newTransaction);
